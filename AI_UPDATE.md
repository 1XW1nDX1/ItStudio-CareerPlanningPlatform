...
### 问题 1：用户能看到更新指令
**现象**：AI 回复中包含 `[UPDATE_RESUME]` 指令，用户可见

**原因**：流式输出时直接发送所有内容给前端

**解决**：在流式输出时实时检测并过滤更新指令标记
- 检测到 `[UPDATE_RESUME]` 时停止发送
- 缓存指令内容
- 检测到 `[/UPDATE_RESUME]` 后继续发送正常内容

### 问题 2：401 未认证错误
**现象**：AI 调用后端 API 时返回 401 错误

**原因**：WebSocket 连接未传递 token，AI 服务无法以用户身份调用后端

**解决**：
1. 前端在 WebSocket URL 中添加 `token` 参数
2. AI 服务接收 `token` 参数并传递给后端 API
